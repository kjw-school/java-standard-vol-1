...
public class Chapter02_2 {
	 ...
	 * 자료형 | 저장 가능한 값의 범위 | bit | byte<br>
	 * booelan | false, true | 8 | 1<br>
	 * char | '\u0000' ~ '\uffff'(0~2^16-1, 0~65535) | 16 | 2<br>
	 * byte | -128 ~ -127(-2^7 ~ 2^7-1) | 8 | 1<br>
	 * short | -32,768 ~ 32,767 (-2^15 ~ 2^15-1) 16 | 2<br>
	 * int | -2,147,483,648 ~ 2,147,483,647 (-2^31 ~ 2^31-1, 약 +- 20억) | 32 | 4<br>
	 * long | -9,223,372,036,854,775,808 ~ 9,223,372,036,854,775,807(-2^63 ~ 2^63-1) | 64 | 8<br>
	 * float | 1.4E-45 ~ 3.4E38(1.4x10^-45 ~ 3.4x10^38) | 32 | 4<br>
	 * double | 4.9E-324 ~ 1.8E308(4.9x10^-324 ~ 1.8x10^308) | 64 | 8<br>
	 * 1 bye = 8 bit<br>
	 ...
	 * 종류 | 리터럴 | 접미사<br>
	 * 논리형 | false, true | 없음<br>
	 * 정수형 | 123, 0b0101, 077, 0xFF, 100L | L<br>
	 * 실수형 | 3.14, 3.0e8, 1.4f, 0x1, 0p-1 | f,d<br>
	 * 문자형 | 'A', '1', '\n' | 없음<br>
	 * 문자열 | "ABC", "123", "A", "true" | 없음<br>
	 * long타입의 리터럴에 접미사 '1'또는 'L'을 붙ㄴ이고, 접미사가 없으면 int타입의 리터럴이다.<br>
	...
	/**
	 * <h5>타입의 불일치</h5><br>
	 * 리터럴의 타입은 저장될 변수의 타입과 일치하는 것이 보통이지만, 타입이 달라도 저장범위가 넓은 타입에 좁은 타입의 값을 저장하는 것은허용된다.<br>
	 ...
	 * 지시자 | 설명<br>
	 * %b | 불리언(boolean) 형식으로 출력<br>
	 * %d | 10진(decimal) 정수의 형식으로 출력<br>
	 * %o | 8진(octal) 정수의 형식으로 출력<br>
	 * %x, %X | 16진(hexa-decimal) 정수의 형식으로 출력<br>
	 * %f | 부동 소수점(floating-point)의 형식으로 출력<br>
	 * %e, %E | 지수(exponent) 표현식의 형식으로 출력<br>
	 * %c | 문자(character)로 출력<br>
	 * %s | 문자열(string)로 출력<br>
	 */
	class Memo06 {
