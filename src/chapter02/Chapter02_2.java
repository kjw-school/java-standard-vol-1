...
public class Chapter02_2 {
    ...
    /**
     * 2.3 형식화된 출력 - printf()
     * printf()는 '지시자(specifier)'를 통해 변수의 값을 여러 가지 형식으로 변환하여 출력하는 기능을 가지고 있다. '지시자'는 값을 어떻게
     * 출력할 것인지를 지정해주는 역할을 한다. 정수형 변수에 저장된 값을 10진 정수로 출력할 때는 지시자 '%d'를 사용하며, 변수의 값을 지정된
     * 형식으로 변환해서 지시자대신 넣는다.
     * 지시자 | 설명
