...
public class Chapter02_4 {
	 ...
	 * 정수형에는 모두 4개의 자료형이 있으며, 각 자료형이 저잘할 수 잇는 값의 범위가 서로 다르다.<br>
	 ...
	 * </table>
	 * <br>
	 * float타입의 표현범위는 '-3.4x10^38 ~ 3.4x10^38'이지만, '-1.4x10^-45 ~ 1.4x10^-45'범위(0은 제외)의 값은 표현할 수 없다.<br>
	...
	/**
	 * <h5>실수형의 저장형식</h5><br>
	 * 실수형은 정수형과 표현형식이 달라서, 실수형은 값을 부동소수점수(floating-point)의 형태로 저장한다. 부동소수점수는 실수를 '+-M x 2^E'와 같은 형태로 표현하는 것을 말하며,<br>
	 ...
	 * 'S는 부호비트(sign bit)를 의미하며 1 bit이다. 이 값이 0이면 양수를, 1이면 음수를 의미한다. 정수형과 달리 '2의 보수법'을 사용하지 않기 때문에 양의 실수를 음의 실수로 바꾸려면 그저 부호비트만 0에서 1로 변경하면 된다.<br>
