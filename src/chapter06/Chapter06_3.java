...
public class Chapter06_3 {
	 ...
	 * </code></pre>
	 *
	 * iv와 cv는 클래스 영역에 선언되어있으므로 멤버변수이다. 그 중 cv는 키워드 static과 함께 선언되어 있으므로 클래스 변수이며, iv는 인스턴스 변수이다.<br>
	 ...
	 *             <td>클래스 영역 이외의 영역(메서드, 생성자, 초기화 블럭 내부)</td>
	 ...
	 * 클래스가 메모리에 '로딩(loading)'될 때 생성되어 프로그램이 종료될 때 까지 유지되며, public을 앞에 붙이면 같은 프로그램내에서 어디서나 접근할 수 있는 '전역변수(global variable)'의 성격을 갖는다.<br>
	 * <small>※참조변수의 선언이나 객체의 생성과 같이 클래스의 정보가 필요할 때, 클래스는 메모리에 로딩된다.</small><br>
